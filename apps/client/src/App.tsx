...
function PlayerArea({ player, position, isCurrent, isYou, pendingCardIds, hasIncomingCard, reducedMotion, onInspectCards }: { readonly player: GameSnapshot["state"]["players"][number]; readonly position: "top" | "left" | "right" | "bottom"; readonly isCurrent: boolean; readonly isYou: boolean; readonly pendingCardIds: ReadonlySet<string>; readonly hasIncomingCard: boolean; readonly reducedMotion: boolean | null; readonly onInspectCards: (cards: readonly CardDefinition[]) => void }) {
  ...
  return <li
    ...
    data-inspectable={cards.length > 0 ? "true" : undefined}
    className={`table-player table-player--${position} table-player--${player.roundStatus} ${isCurrent ? "table-player--active" : ""} ${isYou ? "table-player--you" : ""}`}
    role={cards.length > 0 ? "button" : undefined}
    tabIndex={cards.length > 0 ? 0 : undefined}
    aria-label={cards.length > 0 ? `View all cards held by ${player.name}. ${player.roundStatus === "active" ? isCurrent ? "Taking their turn" : "Still in the round" : outcomeLabel}` : `${player.name}. ${player.roundStatus === "active" ? isCurrent ? "Taking their turn" : "Still in the round" : outcomeLabel}`}
  ...
  >
    ...
    <div className="table-player__hand">{cards.length > 0 ? cards.map((card, index) => <motion.div key={card.id} layout="position" layoutId={dealLayoutId(card.id)} transition={{ layout: { duration: reducedMotion ? 0 : 0.5, ease: [0.22, 1, 0.36, 1] } }} className={`table-player__card ${index === 0 ? "shrink-0" : "-ml-12 shrink-0 sm:-ml-10"}`}><GameCard card={card.definition} size="table" /></motion.div>) : <p>{hasIncomingCard ? "Dealing…" : "Waiting for a card"}</p>}</div>
